...
public class SocksClean {
	public int solution(int K, int[] C, int[] D) {
		int count = 0;
		List<Integer> cl = Arrays.stream(C).boxed().collect(Collectors.toList());
		List<Integer> dl = Arrays.stream(D).boxed().collect(Collectors.toList());
	...
	public static void main(String[] args) {
		...
		System.out.println(new SocksClean().solution(2, C, D));
	}
}
